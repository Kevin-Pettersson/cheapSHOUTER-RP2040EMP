...
void init_buzzer() {
    // Get PWM slice
    uint32_t slice = pwm_gpio_to_slice_num(PIN_BUZZER);
    ...
    // Init PWM, but don't start it yet
    pwm_init(slice, &config, false);
    pwm_set_freq_duty(slice, PWM_CHAN_B, 80, 0.2);
...
void picoemp_init() {
    ...
    // Enable PULSE pulldown
    gpio_set_pulls(PIN_BTN_PULSE, false, true);
